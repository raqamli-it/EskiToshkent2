...
function HomeMonuments() {
  const navigate = useNavigate();
  const [data, setData] = useState([]);

  const getData = async () => {
    const response = await axios.get(`archaeologies/`);
          ...
          <Slider {...settings}>
            {data?.map((value, index) => (
              <div key={index} className="">
                <div
                  key={index}
                  className="w-[94%] bg-white mx-auto group relative sm:w-[90%]"
                >
                  <img
                    src={value?.image}
                    alt={value?.title_uz}
                  ...
                  />
                  <button
                    onClick={() => navigate(`monumentsDetail/${value.id}`)}
                    className="h-[75px] before:ml-[7%] line-clamp-1 px-2 w-full text-[18px] font-semibold 
    before:content-[''] before:absolute before:left-0 before:bottom-5 before:h-[3px] 
    before:bg-black before:transition-all before:duration-300
    hover:before:w-[86%] before:w-0 sm:h-[60px] sm:before:bottom-3"
                  >
                    {value?.title_uz}
                  </button>
                </div>
